...
export default function GlobalCLI({ 
  ...
  return (
    <div className="fixed top-0 left-1/2 -translate-x-1/2 md:left-auto md:right-6 md:translate-x-0 z-50 w-[calc(100%-1rem)] md:w-full md:max-w-md pt-4">
      {!isExpanded ? (
        <div
          onClick={() => setIsExpanded(true)}
          className="w-full glass rounded-2xl p-3 md:p-4 border border-indigo-500/20 hover:border-indigo-500/50 transition-all duration-300 flex items-center justify-between group cursor-pointer"
        >
        ...
        <div className="glass rounded-2xl p-4 md:p-6 border border-indigo-500/20 hover:border-indigo-500/40 transition-colors duration-300 font-mono">
          {/* Terminal Header */}
          ...
          <div
            ref={outputRef}
            className="h-48 md:h-64 overflow-y-auto mb-4 space-y-2 text-xs md:text-sm bg-slate-900/30 rounded-lg p-3 md:p-4 border border-slate-700/50"
          >
            {output.map((item, idx) => (
